...
/**
 * ClientConnections is the first activity for the sample application, it
 * displays all the active connections. After it's loaded it auto-switches
 * to Main Activity
 */
public class ClientConnections extends ListActivity {
   ...
   */
  final static String TOKEN = "com.ferit.davor.smarthouse.ClientConnections";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if(firstCreate==true){
      firstCreate=false;
      try {
        host =arrayAdapter.getItem(0).getHostName()+":"+arrayAdapter.getItem(0).getPort();
  ...
  protected void onListItemClick(ListView listView, View view, int position,
    ...
    if (!contextualActionBarActive) {
      ...
      intent.setClassName(getApplicationContext().getPackageName(),
          "com.ferit.davor.smarthouse.ConnectionDetails");
